...
function create()
    if tasksWindow then destroy() end

    openTasksButton = modules.client_topmenu.addRightGameToggleButton(
                          "openTasksButton", tr("Open Tasks Panel"),
                          "/game_tasks/images/tasks", toggleTasksPanel)
    openTasksButton:setOn(false)

    trackerButton = modules.client_topmenu.addRightGameToggleButton(
                        "trackerButton", tr("Tasks Tracker"),
                        "/game_tasks/images/tracker", toggleTracker)
    trackerButton:setOn(true)

    trackerWindow = g_ui.loadUI("tasks_tracker",
...
function onExtendedOpcode(protocol, code, buffer)
    ...
    local endData = false
    if char == "E" then endData = true end
    ...
    if partialData and not endData then return end
...
function onTasksConfig(data)
    ...
    tasksWindow.info.kills.bar.scroll:setRange(config.kills.Min,
                                               config.kills.Max)
...
function onTasksList(data)
    ...
    for taskId, task in ipairs(data) do
        ...
        if not (task.lvl >= level - config.range and task.lvl <= level +
            config.range) then widget.info.bonus:hide() end
    end
...
function onTasksActive(data)
    for _, active in ipairs(data) do
        ...
        local widget = g_ui.createWidget("TrackerButton",
                                         trackerWindow.contentsPanel
                                             .trackerPanel)
...
function onTaskUpdate(data)
    local widget =
        trackerWindow.contentsPanel.trackerPanel[tostring(data.taskId)]
    if data.status == 1 then
        local task = tasks[data.taskId]
        if not widget then
            widget = g_ui.createWidget("TrackerButton",
                                       trackerWindow.contentsPanel.trackerPanel)
...
function onTrackerClick(widget, mousePosition, mouseButton)
    ...
    menu:addOption("Abandon this task", function() cancel(taskId) end)
...
function onTaskSelected(parent, child, reason)
    ...
    for id, monster in ipairs(task.mobs) do
        local widget =
            g_ui.createWidget("UICreature", tasksWindow.info.monsters)
...
function onKillsValueChange(widget, value, delta)
    ...
    local bonus = math.floor(
                      (math.max(0, value - config.bonus) / config.bonus) + 0.5)
    if bonus == 0 then
        ...
        for _, reward in ipairs(task.rewards) do
            if reward.type == RewardType.Points then
                ...
                tasksWindow.info.kills.bonuses.points:setText(
                    "+" .. finalBonus .. "% Tasks Points")
            elseif reward.type == RewardType.Experience then
                local finalBonus = bonus * config.exp
                tasksWindow.info.kills.bonuses.exp:show()
                tasksWindow.info.kills.bonuses.exp:setText(
                    "+" .. finalBonus .. "% Exp")
            elseif reward.type == RewardType.Gold then
                local finalBonus = bonus * config.gold
                tasksWindow.info.kills.bonuses.gold:show()
                tasksWindow.info.kills.bonuses.gold:setText(
                    "+" .. finalBonus .. "% Gold")
            end
        end
    end
...
function start()
    ...
    if protocolGame then
        protocolGame:sendExtendedOpcode(OPCODE, json.encode(
                                            {
                action = "start",
                data = {taskId = taskId, kills = kills}
            }))
    end
end
...
function cancel(taskId)
    ...
    if protocolGame then
        protocolGame:sendExtendedOpcode(OPCODE, json.encode(
                                            {action = "cancel", data = taskId}))
    end
end

function onTrackerClose() trackerButton:setOn(false) end
...
function toggle()
    if not tasksWindow then return end
    if tasksWindow:isVisible() then return hide() end
    show()
end
...
function show()
    ...
    if playerLevel ~= level then
        ...
        for taskId, child in ipairs(children) do
            local task = tasks[taskId]
            if task.lvl >= level - config.range and task.lvl <= level +
                config.range then
